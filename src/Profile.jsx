...
export default function Profile() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-bg flex items-center justify-center">
      <div className="flex flex-col items-center gap-3 text-violet-400">
        ...
        <span className="text-xs text-ink/30">Loading your profile…</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-bg text-ink">
      ...
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 z-50 bg-bg/80 backdrop-blur-md border-b border-ink/5 flex items-center justify-between px-5 py-3.5">
        <button onClick={() => navigate("/home")} className="w-9 h-9 rounded-xl border border-ink/10 bg-white/5 flex items-center justify-center text-ink/40 hover:text-violet-400 hover:border-violet-500/40 transition text-sm">
          <FontAwesomeIcon icon={faArrowLeft} />
        </button>
        <span className="font-black tracking-tight text-ink">My <span className="text-violet-400">Profile</span></span>
        {!editing
          ? <button onClick={() => setEditing(true)} className="w-9 h-9 rounded-xl border border-ink/10 bg-white/5 flex items-center justify-center text-ink/40 hover:text-violet-400 hover:border-violet-500/40 transition text-sm"><FontAwesomeIcon icon={faPen} /></button>
          : <button onClick={cancelEdit} className="w-9 h-9 rounded-xl border border-ink/10 bg-white/5 flex items-center justify-center text-ink/40 hover:text-pink-400 transition text-sm"><FontAwesomeIcon icon={faTimes} /></button>
        }
  ...
  <ThemeToggle />
      ...
      <main className="relative z-10 pt-20 px-4 pb-10 max-w-xl mx-auto space-y-4">
        ...
        {/* Profile Card */}
        <div className="bg-white/[0.03] border border-ink/10 rounded-3xl p-6 mt-4">
          {/* Avatar */}
          <div className="flex flex-col items-center mb-5">
            <div className="relative mb-3">
              ...
              {editing && (
                <button onClick={() => fileRef.current.click()} className="absolute bottom-0 right-0 w-8 h-8 rounded-full bg-violet-500 hover:bg-violet-400 border-2 border-[#0a0a0f] flex items-center justify-center text-ink text-xs transition">
            ...
            {editing ? (
              ...
              <h2 className="text-xl font-bold text-ink">{pgUser?.displayName || "—"}</h2>
            )}

            <div className="flex items-center gap-2 mt-2 text-ink/30 text-sm">
            ...
            <div className="flex items-center gap-2 mt-2 text-ink/20 text-xs">
          ...
          {editing && (
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-semibold text-ink/30 uppercase tracking-wider mb-1.5">Bio</label>
              ...
              <div className="relative">
                <label className="block text-xs font-semibold text-ink/30 uppercase tracking-wider mb-1.5">
                  <FontAwesomeIcon icon={faBuildingColumns} className="mr-1" /> University
                </label>
                <div className="relative">
                  <FontAwesomeIcon icon={faSearch} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink/20 text-xs pointer-events-none" />
                ...
                {uniOpen && uniSearch.length > 0 && filteredUnis.length > 0 && (
                  <div className="absolute z-10 mt-1 w-full bg-bg-elevated border border-ink/10 rounded-xl overflow-hidden shadow-2xl max-h-48 overflow-y-auto">
                    {filteredUnis.map(u => (
                      <button key={u.id} onClick={() => { setForm(f => ({ ...f, universityId: u.id })); setUniSearch(u.name); setUniOpen(false); }}
                        className={`w-full text-left px-4 py-2.5 text-sm flex items-center gap-2 hover:bg-violet-500/10 hover:text-violet-400 transition ${form.universityId === u.id ? "bg-violet-500/10 text-violet-400" : "text-ink/60"}`}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-ink/30 uppercase tracking-wider mb-1.5"><FontAwesomeIcon icon={faUserGraduate} className="mr-1" />Faculty</label>
                  <input className={inputCls} placeholder="e.g. Engineering" value={form.faculty} onChange={e => setForm(f => ({ ...f, faculty: e.target.value }))} />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-ink/30 uppercase tracking-wider mb-1.5">Department</label>
              ...
              <button onClick={handleSave} disabled={saving} className="w-full py-3 rounded-xl bg-violet-500 hover:bg-violet-400 disabled:opacity-40 text-ink text-sm font-semibold flex items-center justify-center gap-2 transition">
          ...
          {!editing && (
            <>
              {pgUser?.bio && <p className="text-sm text-ink/40 leading-relaxed text-center mb-3">{pgUser.bio}</p>}
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            ...
            <div key={i} className="bg-white/[0.03] border border-ink/10 rounded-2xl p-3 text-center">
              <div className={`text-lg mb-1 ${s.color}`}><FontAwesomeIcon icon={s.icon} /></div>
              <div className="text-lg font-bold text-ink">{s.value}</div>
              <div className="text-xs text-ink/30">{s.label}</div>
            </div>
          ))}
        </div>
        ...
        {stats?.leaderboardScore > 0 && (
          <div className="bg-white/[0.03] border border-ink/10 rounded-2xl p-4 flex items-center gap-4">
            ...
            <div>
              <div className="font-bold text-ink text-base">{stats.leaderboardScore.toLocaleString()} pts</div>
              <div className="text-xs text-ink/30">Leaderboard score</div>
            </div>
            <FontAwesomeIcon icon={faChartBar} className="ml-auto text-ink/20 text-sm" />
          </div>
        )}
        ...
        <div className="flex gap-2">
          {[
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 border transition ${activeTab === tab.id ? "bg-violet-500 text-white border-violet-500" : "bg-white/[0.03] border-ink/10 text-ink/40 hover:border-violet-500/30 hover:text-violet-400"}`}>
        ...
        {activeTab === "materials" && (
          <div className="space-y-3">
            {materials.length === 0 ? (
              <div className="bg-white/[0.03] border border-ink/10 rounded-2xl py-10 text-center">
                ...
                <p className="font-bold text-ink text-sm mb-1">No materials yet</p>
                <p className="text-xs text-ink/30 mb-4">Upload study materials to share with your campus</p>
              ...
              materials.map((m, i) => (
                <div key={m.id} className="bg-white/[0.03] border border-ink/10 rounded-2xl p-4 flex items-center gap-3 hover:border-violet-500/20 transition" style={{ animationDelay: `${i * 0.04}s` }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-ink/30 mt-0.5">{m.faculty || "—"} · {new Date(m.createdAt).toLocaleDateString()}</p>
                  </div>
                  <div className="flex flex-col items-end gap-1.5 shrink-0">
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${m.isPublic ? "bg-emerald-500/15 text-emerald-400" : "bg-white/5 text-ink/30"}`}>
        ...
        {activeTab === "listings" && (
          <div className="bg-white/[0.03] border border-ink/10 rounded-2xl py-10 text-center">
            ...
            <p className="font-bold text-ink text-sm mb-1">No listings yet</p>
            <p className="text-xs text-ink/30 mb-4">List items on the marketplace to sell to fellow students</p>
      ...
      {/* Toast */}
      {saveOk && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-bg-elevated border border-ink/10 text-ink px-5 py-3 rounded-full text-sm font-medium flex items-center gap-2 z-50 shadow-xl animate-bounce-in whitespace-nowrap">
