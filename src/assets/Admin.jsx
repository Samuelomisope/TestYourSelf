            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map(p => (
                <tr key={p.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${p.isAvailable ? "bg-green-100 text-green-600" : "bg-red-100 text-red-500"}`}>
      ...
      {loading ? <p className="text-gray-400 text-sm text-center py-10">Loading...</p> : (
        reports.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {reports.map(r => (
              <div key={r.id} className={`bg-white rounded-2xl p-4 border shadow-sm ${r.resolved ? "opacity-50" : ""}`}>
        ...
        {activeTab === "overview" && (
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              <StatCard label="Total Users" value={stats?.users} icon="👥" color="border-indigo-400" />
              <StatCard label="Study Materials" value={stats?.materials} icon="📚" color="border-purple-400" />
              <StatCard label="Products Listed" value={stats?.products} icon="🛍️" color="border-pink-400" />
              <StatCard label="Universities" value={stats?.universities} icon="🏫" color="border-green-400" />
            </div>
