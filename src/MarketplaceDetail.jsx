    ...
    <div className="flex gap-1">
      {[1,2,3,4,5].map(star => (
        <button key={star} onClick={() => onChange && onChange(star)} type="button">
          <FontAwesomeIcon icon={faStar} className={`text-xl ${star <= value ? "text-yellow-400" : "text-ink/10"} transition`} />
        </button>
      ))}
    </div>
  );
...
function MarketplaceDetail() {
  ...
  if (!item) return <div className="min-h-screen bg-bg flex items-center justify-center text-ink/40">Item not found.</div>;
    ...
    <div className="min-h-screen bg-bg text-ink">
      ...
      <header className="bg-bg/80 backdrop-blur-md border-b border-ink/5 px-4 py-3 sticky top-0 z-40">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <h1 className="text-base font-bold text-ink truncate">{item.title}</h1>
        </div>
      </header>

      <div className="relative z-10 max-w-3xl mx-auto px-4 py-5 space-y-4">
        {/* Images */}
        <div className="bg-white/[0.03] border border-ink/10 rounded-2xl overflow-hidden">
            ...
            {item.images?.length > 0 ? (
              <>
                ...
                {item.images.length > 1 && (
                  <>
                    <button onClick={() => setImgIndex(i => Math.max(0, i - 1))} className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/50 rounded-full w-8 h-8 flex items-center justify-center text-ink/60 hover:text-ink transition"><FontAwesomeIcon icon={faChevronLeft} className="text-sm" /></button>
                    <button onClick={() => setImgIndex(i => Math.min(item.images.length - 1, i + 1))} className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/50 rounded-full w-8 h-8 flex items-center justify-center text-ink/60 hover:text-ink transition"><FontAwesomeIcon icon={faChevronRight} className="text-sm" /></button>
              ...
              <div className="w-full h-full flex items-center justify-center text-ink/10"><FontAwesomeIcon icon={faBoxOpen} className="text-5xl" /></div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white/[0.03] border border-ink/10 rounded-2xl p-5">
          ...
          <p className="text-ink/50 text-sm mt-3 leading-relaxed">{item.description}</p>
          {item.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-3">
              {item.tags.map(tag => <span key={tag} className="flex items-center gap-1 px-2 py-0.5 bg-white/5 text-ink/40 rounded-full text-xs"><FontAwesomeIcon icon={faTag} /> {tag}</span>)}
            </div>
          )}
          <div className="flex items-center gap-4 mt-4 text-xs text-ink/30">
        ...
        {/* Seller */}
        <div className="bg-white/[0.03] border border-ink/10 rounded-2xl p-5">
          <p className="text-sm font-semibold text-ink/50 mb-3 flex items-center gap-2"><FontAwesomeIcon icon={faStore} className="text-violet-400" /> Seller</p>
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => navigate(`/marketplace/seller/${seller.id}`)}>
              <img src={seller.photoURL || `https://api.dicebear.com/7.x/initials/svg?seed=${seller.displayName}`} className="w-10 h-10 rounded-full object-cover border border-ink/10" alt="" />
              <div>
                <p className="font-medium text-ink text-sm">{seller.displayName}</p>
...
/>
        ...
        {/* Reviews */}
        <div className="bg-white/[0.03] border border-ink/10 rounded-2xl p-5">
          <p className="text-sm font-semibold text-ink/50 mb-4 flex items-center gap-2"><FontAwesomeIcon icon={faStar} className="text-yellow-400" /> Reviews ({item.reviews?.length || 0})</p>
          <div className="mb-5 pb-5 border-b border-ink/5">
            <p className="text-xs text-ink/30 mb-2">Leave a review</p>
            <StarRating value={rating} onChange={setRating} />
            <textarea value={comment} onChange={e => setComment(e.target.value)} placeholder="Write a comment (optional)..." rows={2}
              className="w-full mt-2 bg-black/40 border border-ink/10 rounded-xl px-3 py-2 text-sm text-ink placeholder-white/20 outline-none focus:border-violet-500/40 resize-none transition" />
          ...
          {item.reviews?.length === 0 ? (
            <p className="text-ink/20 text-sm text-center py-4">No reviews yet. Be the first!</p>
          ) : (
            <div className="space-y-3">
              {item.reviews.map((r, i) => (
                <div key={i} className="flex gap-3">
                  <img src={r.user?.photoURL || `https://api.dicebear.com/7.x/initials/svg?seed=${r.user?.displayName}`} className="w-8 h-8 rounded-full object-cover border border-ink/10 shrink-0" alt="" />
                  <div>
                    <div className="flex items-center gap-2"><p className="text-sm font-medium text-ink">{r.user?.displayName}</p><StarRating value={r.rating} /></div>
                    {r.comment && <p className="text-xs text-ink/40 mt-0.5">{r.comment}</p>}
                    <p className="text-xs text-ink/20 mt-0.5">{new Date(r.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
